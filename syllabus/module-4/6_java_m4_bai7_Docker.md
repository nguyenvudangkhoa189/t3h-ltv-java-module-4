...
| URL                                                                            | Ai                   | Kỳ vọng                        |
| ------------------------------------------------------------------------------ | -------------------- | ------------------------------ |
| [http://localhost:8080/movies](http://localhost:8080/movies)                   | Khách                | Public (đã import `mymoviedb`) |
| [http://localhost:8080/login](http://localhost:8080/login)                     | —                    | Form login                     |
| [http://localhost:8080/admin/dashboard](http://localhost:8080/admin/dashboard) | `admin` / `admin123` | Dashboard D1–D6                |
...
1. Chạy Mongo **không** mật khẩu rồi **có** mật khẩu; viết đúng hai URI Spring tương ứng.
2. (Tuỳ chọn) Chạy MySQL có mật khẩu, `SHOW DATABASES` bằng `docker exec`.

**Lab 2**

1. Nêu **ba lý do** dùng multi-stage thay vì một stage Maven+JDK để chạy app.
2. Vẽ: trình duyệt → port host → app → hostname `mongo`.
3. `compose down` không `-v` rồi `up`: data còn không? Thử `-v` (lab).
4. (Tuỳ chọn) `docker compose logs app` khi lỗi URI.
...
- Dockerfile / multi-stage: [https://docs.docker.com/build/building/multi-stage/](https://docs.docker.com/build/building/multi-stage/)  
- Compose: [https://docs.docker.com/compose/](https://docs.docker.com/compose/)  
- Image Mongo: [https://hub.docker.com/_/mongo](https://hub.docker.com/_/mongo) · MySQL: [https://hub.docker.com/_/mysql](https://hub.docker.com/_/mysql)  
- Temurin: [https://hub.docker.com/_/eclipse-temurin](https://hub.docker.com/_/eclipse-temurin)  
- Spring Boot Externalized Config: [https://docs.spring.io/spring-boot/reference/features/external-config.html](https://docs.spring.io/spring-boot/reference/features/external-config.html)  
